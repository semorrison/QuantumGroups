...
\!\(InterpolationInverse[mat_?SquareMatrixQ] := Module[{size, newMatrix, det, degree, n, abcissa, data, inverse}, \[IndentingNewLine]DebugPrint["\<inverting matrix of size \>", Length[mat]]; \[IndentingNewLine]size = Length[mat]; \[IndentingNewLine]det = Simplify[Det[mat]]; \[IndentingNewLine]newMatrix = mat\/det; \[IndentingNewLine]degree = Min[Apply[Plus, Map[Max[Exponent[#, q]] &, mat]], Apply[Plus, Map[Max[Exponent[#, q]] &, Transpose[mat]]]]; \[IndentingNewLine]If[degree \[Equal] 0, Return[Inverse[mat]]]; \[IndentingNewLine]abcissa = {}; \[IndentingNewLine]n = Floor[\(-\(degree\/2\)\)] + 1; \[IndentingNewLine]While[Length[abcissa] < degree + 2, \[IndentingNewLine]If[\((det /. q \[Rule] n)\) =!= 0, abcissa = Append[abcissa, n]]; \[IndentingNewLine]\(n++\)\[IndentingNewLine]]; \[IndentingNewLine]If[size > 20, DebugPrint["\<Inverting numerical matrices:\>"]]; \[IndentingNewLine]data = Transpose[Table[If[size > 20, DebugPrint[i]]; Inverse[newMatrix /. q \[Rule] abcissa\[LeftDoubleBracket]i\[RightDoubleBracket]], {i, 1, Length[abcissa]}], {3, 1, 2}]; \[IndentingNewLine]If[size > 20, DebugPrint["\<Interpolating numerical matrices:\>"]]; \[IndentingNewLine]inverse = Table[If[i \[Equal] 1 \[And] size > 20, DebugPrint[j]]; Simplify[InterpolatingPolynomial[Transpose[{abcissa, data\[LeftDoubleBracket]i, j\[RightDoubleBracket]}], q]], {i, 1, size}, {j, 1, size}]; \[IndentingNewLine]DebugPrint["\<done\>"]; \[IndentingNewLine]Together[\(1\/det\) inverse]\[IndentingNewLine]]\)
