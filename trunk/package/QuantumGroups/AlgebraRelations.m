...
\!\(SerreRelations[\[CapitalGamma]_] := With[{cm = CartanMatrix[\[CapitalGamma]], cf = CartanFactors[\[CapitalGamma]], n = Rank[\[CapitalGamma]]}, \[IndentingNewLine]DeleteCases[Flatten[Join[\[IndentingNewLine]Table[\[Sum]\+\(r = 0\)\%\(1 - cm\[LeftDoubleBracket]i, j\[RightDoubleBracket]\)\(\((\(-1\))\)\^r\) \(qBinomial[1 - cm\[LeftDoubleBracket]i, j\[RightDoubleBracket], r]\)[q\^cf\[LeftDoubleBracket]i\[RightDoubleBracket]] \((\(X\_i\^+\))\)\^\(\(**\)\((1 - cm\[LeftDoubleBracket]i, j\[RightDoubleBracket] - r)\)\) ** \(X\_j\^+\) ** \((\(X\_i\^+\))\)\^\(\(**\)\(r\)\) \[Equal] \[ScriptZero], {i, 1, n}, {j, 1, n}], \[IndentingNewLine]Table[\[Sum]\+\(r = 0\)\%\(1 - cm\[LeftDoubleBracket]i, j\[RightDoubleBracket]\)\(\((\(-1\))\)\^r\) \(qBinomial[1 - cm\[LeftDoubleBracket]i, j\[RightDoubleBracket], r]\)[q\^cf\[LeftDoubleBracket]i\[RightDoubleBracket]] \((\(X\_i\^-\))\)\^\(\(**\)\((1 - cm\[LeftDoubleBracket]i, j\[RightDoubleBracket] - r)\)\) ** \(X\_j\^-\) ** \((\(X\_i\^-\))\)\^\(\(**\)\(r\)\) \[Equal] \[ScriptZero], {i, 1, n}, {j, 1, n}]\[IndentingNewLine]]] /. {0 \[Rule] \[ScriptZero]}, True]\[IndentingNewLine]]\)
